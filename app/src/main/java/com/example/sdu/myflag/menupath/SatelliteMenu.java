...
public class SatelliteMenu extends FrameLayout {
	...
	public void addItems(List<SatelliteMenuItem> items) {
		...
		tmpView.setLayoutParams(new LayoutParams(0, 0));
		...
		for (SatelliteMenuItem menuItem : menuItems) {
			...
			LayoutParams layoutParams = getLayoutParams(cloneView);
	...
	private static LayoutParams getLayoutParams(View view) {
		return (LayoutParams) view.getLayoutParams();
	}

	private static class InternalSatelliteOnClickListener implements OnClickListener {
		private WeakReference<SatelliteMenu> menuRef;

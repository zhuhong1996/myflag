...
public class CommunityFragment extends BaseFragment {
    ...
    private class GetFlagResult implements NetUtil.CallBackForResult {
        ...
        public void onFailure(final IOException e) {
            CommunityFragment.this.getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(CommunityFragment.this.getActivity(), e.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        }
                ...
                try {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        for (int j = 0; j < SuperviseBrief.length(); j++) {
                            ...
                            isSupervise = CommunityFragment.this.id.equals(uid);
                    ...
                    CommunityFragment.this.getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            listAdapter = new FlagFriendListAdapter(CommunityFragment.this.getContext(), list);
